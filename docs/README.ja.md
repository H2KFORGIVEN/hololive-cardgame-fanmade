...
- **初心者チュートリアル** — 5レッスンのガイド付き体験（配置 → ブルーム → コラボ → アーツ → 勝利）
- **Pixi.js WebGL エフェクト** — パーティクル、衝撃波、アタックビーム、スパークルを DOM の上にオーバーレイ
...
| VFX | Pixi.js v8（WebGL オーバーレイ、CDN から ESM で読み込み） |
...
- [x] Phase 2C — 5レッスン式インタラクティブチュートリアル
- [x] Phase 2D — Pixi.js WebGL パーティクルエフェクト層
